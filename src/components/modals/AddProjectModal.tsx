...
export const AddProjectModal = ({ open, onOpenChange }: AddProjectModalProps) => {
  ...
  const [installments, setInstallments] = useState<InstallmentPayment[]>([
    { id: '1', percentage: 0, dueDate: undefined, amount: 0, status: 'pending' }
  ]);
  ...
  const fetchWonOpportunities = async () => {
    ...
    try {
      ...
      const wonOpportunities = data?.map(item => ({
  ...
  const updateInstallment = (id: string, field: keyof InstallmentPayment, value: any) => {
  ...
  const isFormValid = () => {
    const basicFields = formData.projectName && formData.poNumber && formData.poDate && formData.poAmount && formData.paymentType;
    
    if (!basicFields) return false;

    if (formData.paymentType === 'installment') {
      return getTotalPercentage() === 100 && installments.every(inst => inst.dueDate);
    }
  ...
  const handleSubmit = () => {
    if (isFormValid()) {
      ...
      setInstallments([{ id: '1', percentage: 0, dueDate: undefined, amount: 0, status: 'pending' }]);
    }
  };
  ...
  }, [formData.poAmount, installments.map(inst => inst.percentage).join(',')]);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.projectName} onValueChange={(value) => setFormData({...formData, projectName: value})} disabled={loading}>
              <SelectTrigger>
                <SelectValue placeholder={loading ? "Loading won opportunities..." : "Select won opportunity"} />
              </SelectTrigger>
              <SelectContent>
                {wonOpportunities.map((opportunity) => (
                  <SelectItem key={opportunity.id} value={opportunity.name}>
                    {opportunity.name}
                  </SelectItem>
                ))}
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {formData.poDate ? format(formData.poDate, "PPP") : "Select PO date"}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  ...
                  onSelect={(date) => setFormData({...formData, poDate: date})}
                  initialFocus
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {formData.paymentDate ? format(formData.paymentDate, "PPP") : "Select payment date"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
          ...
          {formData.paymentType === 'top' && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={formData.topDays} onValueChange={(value) => setFormData({...formData, topDays: value})}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {formData.paymentDate ? format(formData.paymentDate, "PPP") : "Select due date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {installments.map((installment, index) => (
                  <div key={installment.id} className="grid grid-cols-5 gap-3 items-end p-3 border rounded-lg">
                    <div>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            ...
                            {installment.dueDate ? format(installment.dueDate, "dd/MM") : "Date"}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0" align="start">
                          <Calendar
                    ...
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => updateInstallment(installment.id, 'status', value)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
