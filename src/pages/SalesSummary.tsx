...
const SalesSummary = () => {
  ...
  const [targetAchievement, setTargetAchievement] = useState({ actual: 0, target: 0, percentage: 0 });
  ...
  const [activitySummary, setActivitySummary] = useState({ calls: 0, meetings: 0, visits: 0 });
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const { data: opportunities, error: oppError } = await supabase
          .from('opportunities')
          .select('amount, status, stage, is_won, is_closed, expected_close_date')
          .eq('owner_id', user.id);

        if (oppError) throw oppError;

        // Calculate total revenue (won opportunities only)
        const wonAmountForRevenue = opportunities
          ?.filter(opp => opp.status === 'won' || (opp as any).is_won === true)
        ...
        const wonDeals = opportunities?.filter(
          (opp: any) => opp.status === 'won' || opp.is_won === true || opp.stage === 'Closed Won'
        ).length || 0;
        ...
        const wonAmount = opportunities?.filter(opp => opp.status === 'won' || (opp as any).is_won === true)
          .reduce((sum, opp) => sum + (opp.amount || 0), 0) || 0;

        // Fetch sales target for current user with better error handling
        console.log('Fetching sales targets for user:', user.id);
        ...
        const percentage = targetAmount > 0 ? (wonAmount / targetAmount) * 100 : 0;
        ...
        if (!pipelineError && pipelineItems) {
          const pipelineTotal = pipelineItems.reduce((sum, item: any) => sum + (item.amount || 0), 0);
          setPipelineValue({ total: pipelineTotal, count: pipelineItems.length });

          const map: Record<string, { amount: number; count: number }> = {};
          PIPELINE_SIMPLE_STAGES.forEach(k => { map[k] = { amount: 0, count: 0 }; });
        ...
        const upcomingDeals = opportunities?.filter(opp => {
          if (!opp.expected_close_date) return false;
          const closeDate = new Date(opp.expected_close_date);
          const today = new Date();
          const diffDays = Math.ceil((closeDate.getTime() - today.getTime()) / (1000 * 3600 * 24));
          return diffDays <= 14 && diffDays >= 0;
        }) || [];

        const alertsData = [
          { count: upcomingDeals.length, text: "opportunities within 14 days of expected close date" }
        ];
        ...
        try {
          if (myProfile?.role === 'manager') {
            ...
            const teamIds = (teamMembers || []).map((m: any) => m.account_manager_id);
            allowedOwnerIds = Array.from(new Set([user.id, ...teamIds]));
          } else if (myProfile?.role === 'head' || myProfile?.role === 'admin') {
            const { data: entityMembers } = await supabase
              .from('user_profiles')
              .select('id')
              .eq('entity_id', myProfile.entity_id);
            allowedOwnerIds = (entityMembers || []).map((m: any) => m.id);
          } else {
        ...
        const ownerIds = Array.from(new Set((stageMetrics || [])
        ...
        const ownersMap = new Map((ownerProfiles || []).map((p: any) => [p.id, p]));

        const filteredMetrics = (stageMetrics || []).filter((m: any) => {
          const ownerId = m.owner_id;
          const owner = ownerId ? ownersMap.get(ownerId) : null;
          return owner && (!myProfile?.entity_id || owner.entity_id === myProfile.entity_id);
        });

        // Agregasi poin per pemilik (owner). Poin hanya dihitung jika tidak overdue dan bukan 'Closed Lost'.
        const performerPoints = new Map<string, { name: string; points: number; items: number }>();
        filteredMetrics.forEach((m: any) => {
          ...
          const existing = performerPoints.get(ownerId) || { name: ownerName, points: 0, items: 0 };
        ...
        if (!topPerformersReal || topPerformersReal.length === 0) {
          ...
          const myPoints = (myMetrics || []).reduce((sum: number, m: any) => {
            if (m.stage === 'Closed Lost') return sum;
            const award = m.is_overdue ? 0 : (m.points || 0);
            return sum + award;
          }, 0);

          const displayName = myProfile ? (myProfile as any).full_name || 'Anda' : 'Anda';
          topPerformersReal = myPoints > 0 ? [{ rank: 1, name: displayName, score: `${myPoints} pts` }] : [];
        }
  ...
  const arMetrics = [
    { title: "Balance", value: "16.8M", icon: DollarSign, color: "text-blue-600" },
    { title: "Within Due", value: "7.8M", icon: Calendar, color: "text-green-600" },
    { title: "Overdue", value: "9.0M", icon: Calendar, color: "text-orange-600" },
    { title: "Overdue %", value: "53.6%", icon: Percent, color: "text-red-600" },
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            ...
            <div className="text-sm text-muted-foreground">Won {dealsClosed.won} / Lost {dealsClosed.lost}</div>
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base font-medium">Target vs Achievement</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
              <Progress value={targetAchievement.percentage} className="h-3" />
              <div className="flex justify-between mt-2">
                <span className="text-sm">{targetAchievement.percentage.toFixed(1)}%</span>
              </div>
            </div>
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>Actual: {formatCurrency(targetAchievement.actual)}</span>
              <span>Target: {formatCurrency(targetAchievement.target)}</span>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-2 text-sm">
              {dealsByStage.map((s: any) => (
                <div key={s.name} className="flex items-center justify-between">
                  <span>{s.name}</span>
                  <span>{(s.count || 0)} deals • {formatCurrency(s.amount || 0)}</span>
                </div>
              ))}
            </div>
          </CardContent>
                ...
                <div key={performer.rank} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Badge 
                      ...
                      className={performer.rank === 1 ? "bg-yellow-500" : performer.rank === 2 ? "bg-gray-400" : performer.rank === 3 ? "bg-amber-600" : ""}
                    >
                      {performer.rank}
                    </Badge>
            ...
            <div className="space-y-4">
              {alerts.map((alert, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-orange-500">{alert.count}</span>
      ...
      <div className="mt-12">
        <h2 className="text-3xl font-bold text-foreground mb-8">Accounts Receivable Overview</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base font-medium text-blue-600">Age Analysis of Due Balance</CardTitle>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base font-medium text-blue-600">Overdue Breakup by Region</CardTitle>
              <p className="text-sm text-muted-foreground">(Million USD)</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base font-medium text-blue-600">Outstanding by Status</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="lg:col-span-1">
            <CardHeader>
              <CardTitle className="text-base font-medium text-blue-600">Credit Sales vs Balance Due Trend T13M</CardTitle>
              <p className="text-sm text-muted-foreground">(Million USD)</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={trendData}>
                  ...
                  <YAxis />
                  <Legend />
                  <Bar dataKey="withinDue" stackId="a" fill="#3b82f6" name="Within Due" />
                  <Bar dataKey="overdue" stackId="a" fill="#ef4444" name="Overdue" />
                  <Line dataKey="creditSales" stroke="#f59e0b" name="Credit Sales" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card className="lg:col-span-1">
            <CardHeader>
              <CardTitle className="text-base font-medium text-blue-600">Actual vs Estimated Cash Inflow With 3 Months Forecast</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={cashFlowData}>
                  ...
                  <Bar dataKey="estimated" fill="#3b82f6" name="Estimated Receipts" />
                  <Bar dataKey="forecasted" fill="#f59e0b" name="Forecasted Receipts" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
