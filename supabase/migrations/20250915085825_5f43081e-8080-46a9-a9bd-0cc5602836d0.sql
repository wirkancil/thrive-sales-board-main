-- Drop and recreate admin_update_profile function with correct parameter order
DROP FUNCTION IF EXISTS public.admin_update_profile(uuid, text, uuid, uuid);

CREATE OR REPLACE FUNCTION public.admin_update_profile(p_department uuid, p_division uuid, p_id uuid, p_role text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
