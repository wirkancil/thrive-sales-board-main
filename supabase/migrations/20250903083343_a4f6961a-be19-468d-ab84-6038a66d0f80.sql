-- Option A Migration (retry, idempotent): augment deals, convert status to enum safely if needed, add FKs/triggers
...
-- 3) Backfill created_by from existing user_id
UPDATE public.deals SET created_by = user_id WHERE created_by IS NULL;

-- 4) Ensure enum exists and convert status if not already enum; otherwise just enforce defaults/nulls
DO $$
DECLARE
  _data_type text;
  _udt_name text;
BEGIN
  ...
  IF _data_type = 'USER-DEFINED' AND _udt_name = 'deal_status' THEN
    -- Already enum: just ensure default and null cleanup
    ALTER TABLE public.deals ALTER COLUMN status SET DEFAULT 'warm'::public.deal_status;
    UPDATE public.deals SET status = 'warm'::public.deal_status WHERE status IS NULL;
  ELSE
    -- Still text (or other): normalize then convert
...
-- 6) Triggers: created_by/user_id and updated_at
