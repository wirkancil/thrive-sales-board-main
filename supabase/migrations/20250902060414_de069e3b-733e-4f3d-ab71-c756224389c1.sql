...
CREATE TABLE public.opportunities (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  CONSTRAINT check_customer_is_customer CHECK (
    EXISTS (SELECT 1 FROM public.organizations WHERE id = customer_id AND type = 'customer')
  ),
  CONSTRAINT check_end_user_is_end_user CHECK (
    end_user_id IS NULL OR 
    EXISTS (SELECT 1 FROM public.organizations WHERE id = end_user_id AND type = 'end_user')
  )
);
...
-- SEED DATA for default pipeline
INSERT INTO public.pipelines (name, description, created_by) VALUES 
('Sales Pipeline', 'Standard sales pipeline for opportunities', (SELECT id FROM public.user_profiles WHERE role = 'admin' LIMIT 1));
