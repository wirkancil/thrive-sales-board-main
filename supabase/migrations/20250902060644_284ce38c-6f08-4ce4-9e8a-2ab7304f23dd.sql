-- Master Data CRM Module (Revised - without subquery constraints)
-- Create comprehensive master data structure for CRM

-- 1. ORGANIZATIONS TABLE (for both customers and end users)
...
-- 6. OPPORTUNITY STAGE HISTORY TABLE
...
CREATE INDEX idx_organization_contacts_is_primary ON public.organization_contacts(is_primary);
...
TO authenticated
USING (true); -- Start permissive, can restrict by org_id later
...
-- SEED DATA for default pipeline (with conditional check)
DO $$
DECLARE
    admin_user_id UUID;
    pipeline_id UUID;
BEGIN
    -- Get an admin user for pipeline creation
    ...
    -- Only insert if admin user exists and pipeline doesn't exist
    IF admin_user_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.pipelines WHERE name = 'Sales Pipeline') THEN
        INSERT INTO public.pipelines (name, description, created_by) VALUES 
        ('Sales Pipeline', 'Standard sales pipeline for opportunities', admin_user_id)
        RETURNING id INTO pipeline_id;
        
        -- Insert pipeline stages
        INSERT INTO public.pipeline_stages (pipeline_id, name, sort_order, default_probability, is_won, is_lost) VALUES 
        (pipeline_id, 'Qualification', 1, 10.00, false, false),
        (pipeline_id, 'Proposal', 2, 35.00, false, false),
        (pipeline_id, 'Negotiation', 3, 65.00, false, false),
        (pipeline_id, 'Won', 4, 100.00, true, false),
        (pipeline_id, 'Lost', 5, 0.00, false, true);
    END IF;
END
$$;
...
-- Validation functions for data integrity (replaces check constraints)
CREATE OR REPLACE FUNCTION public.validate_opportunity_relationships()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Validate customer is of type 'customer'
  IF NOT EXISTS (SELECT 1 FROM public.organizations WHERE id = NEW.customer_id AND type = 'customer') THEN
    RAISE EXCEPTION 'Customer must be of type customer';
  END IF;
  
  -- Validate end_user is of type 'end_user' if provided
  IF NEW.end_user_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.organizations WHERE id = NEW.end_user_id AND type = 'end_user') THEN
    RAISE EXCEPTION 'End user must be of type end_user';
  END IF;
...
-- Trigger for opportunity validation
CREATE TRIGGER validate_opportunity_relationships_trigger
  BEFORE INSERT OR UPDATE ON public.opportunities
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_opportunity_relationships();