...
-- 1) Extend user_profiles with department_id and division_id
...
create trigger sales_targets_touch before update on public.sales_targets for each row execute procedure public.tg_sales_targets_touch();

alter table public.sales_targets enable row level security;

-- Reset any existing policies
drop policy if exists "st_admin_all" on public.sales_targets;
