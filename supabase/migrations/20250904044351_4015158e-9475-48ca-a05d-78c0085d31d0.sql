-- Add end_user_mode to opportunities table
ALTER TABLE public.opportunities 
ADD COLUMN IF NOT EXISTS end_user_mode text CHECK (end_user_mode IN ('same_as_customer', 'new_record'));

-- Update opportunities to make end_user_id NOT NULL after setting defaults
UPDATE public.opportunities 
SET end_user_id = customer_id 
WHERE end_user_id IS NULL;

UPDATE public.opportunities 
SET end_user_mode = 'same_as_customer' 
WHERE end_user_mode IS NULL;

ALTER TABLE public.opportunities 
ALTER COLUMN end_user_id SET NOT NULL;

-- Create function to auto-create/reuse end user when mode is same_as_customer
...
-- Create trigger for end user creation
DROP TRIGGER IF EXISTS handle_end_user_creation_trigger ON public.opportunities;
CREATE TRIGGER handle_end_user_creation_trigger
  BEFORE INSERT ON public.opportunities
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_end_user_creation();

-- Create function to check pipeline item close date and create notifications
...
-- Create trigger for pipeline item close date notifications
DROP TRIGGER IF EXISTS check_pipeline_close_date_trigger ON public.pipeline_items;
CREATE TRIGGER check_pipeline_close_date_trigger
  AFTER INSERT OR UPDATE OF expected_close_date ON public.pipeline_items
  FOR EACH ROW
  EXECUTE FUNCTION public.check_pipeline_close_date();

-- Update RLS policies for approval matrix

-- Organizations (Customer/End User) - Account Manager input, Admin approval
DROP POLICY IF EXISTS "Account managers can create organizations" ON public.organizations;
CREATE POLICY "Account managers can create organizations" 
...
-- Opportunities - Account Manager input, Division Head approval
DROP POLICY IF EXISTS "Division heads can approve opportunities" ON public.opportunities;
CREATE POLICY "Division heads can approve opportunities"
ON public.opportunities FOR UPDATE
USING (
  EXISTS (
    SELECT 1 
    FROM user_profiles up_owner
    JOIN user_profiles up_current ON up_current.id = auth.uid()
    WHERE up_owner.id = opportunities.owner_id
    AND (
      up_current.role = 'division_head' 
...
-- Pipeline Items - Account Manager input, Division Head approval (already exists)

-- Customer Organizational Units - Account Manager input, Admin approval
DROP POLICY IF EXISTS "Admins can update all customer org units" ON public.customer_org_units;
CREATE POLICY "Admins can approve customer org units"
ON public.customer_org_units FOR UPDATE
USING (is_admin(auth.uid()));

-- Customer Contacts - Account Manager input, Admin approval  
DROP POLICY IF EXISTS "Admins can update all customer contacts" ON public.customer_contacts;
CREATE POLICY "Admins can approve customer contacts"
...
-- Create view for customer organizational units
...
-- Create view for customer contacts
