-- Drop the function completely with all signatures
DROP FUNCTION IF EXISTS public.get_users_with_profiles();
DROP FUNCTION IF EXISTS public.get_users_with_profiles(text);
DROP FUNCTION IF EXISTS public.get_users_with_profiles(text, text);

-- Create the new function with search and role filter parameters
